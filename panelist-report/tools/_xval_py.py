...
out={
 ...
 "prc":[r["volume"]["primary_raw_clean"]["excluded_cases"],[[x["label"],x["count"]] for x in r["volume"]["primary_raw_clean"]["rows"]]],
 ...
 "drivers":[[x["rank"],x.get("volume_rank"),bool(x.get("pinned")),x["label"],x["count"],x["pct"]] for x in r["drivers"]],
 ...
 "q_junk":[[x["label"],x["count"],[[e["position"],e["cell"]] for e in x.get("examples",[])]] for x in r["quality"]["junk_labels"]],
 "q_unk":[[x["label"],x["count"],[[e["position"],e["cell"]] for e in x.get("examples",[])]] for x in r["quality"]["unknown_labels"]],
 "hdrs":[[[list(h) for h in p.get("headers",[])],p.get("unmapped",[])] for p in prov],
 ...
 "dd":[[d["id"],d.get("facet_set"),d["cases"],d["pct_of_total"],d.get("with_free_text"),
       ...
       [d.get("by_source"),d.get("categories_charted"),d.get("categories_excluded")],
       ([d["trend"]["mom_change"],d["trend"]["mom_pct"],d["trend"]["window_pct"],d["trend"]["total_window_pct"],d["trend"]["peak_key"],d["trend"]["peak_value"],d["trend"]["peak_ratio"],d["trend"]["share_first"],d["trend"]["share_last"],d["trend"]["share_change"],d["trend"]["months"]] if d.get("trend",{}).get("computable") else "NC"),
